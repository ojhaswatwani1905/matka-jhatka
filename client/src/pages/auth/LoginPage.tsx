...
export default function LoginPage() {
  ...
  return (
    <AuthLayout
      ...
      activeMode="login"
    >
      {/* Instant Demo Access Box (Simplified, quiet dark tint) */}
      <div className="p-3.5 rounded-xl bg-amber-500/5 border border-amber-500/20 text-center space-y-2 mb-6">
        ...
        <button
          ...
          disabled={isLoading}
          className="w-full py-2 rounded-lg font-bold text-xs text-black bg-gold hover:bg-amber-400 flex items-center justify-center gap-2 cursor-pointer transition-colors"
        >
      ...
      <div className="my-6 flex items-center gap-3">
        ...
        <span className="text-[10px] font-bold uppercase text-slate-500 tracking-wider">or continue with credentials</span>
        <div className="flex-1 h-px bg-white/10" />
      </div>
      ...
      <AnimatePresence>
        {serverError && (
          <motion.div
            ...
            className="p-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs font-medium mb-6"
          >
            {serverError}
          </motion.div>
        )}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 text-xs">
        ...
        <div className="flex w-full items-center gap-1 bg-slate-950 p-1 rounded-xl border border-slate-800 relative mb-6">
          <button
            type="button"
            onClick={() => setLoginMethod('email')}
            className={`relative z-10 flex-1 w-1/2 py-2 rounded-lg font-bold transition-colors cursor-pointer text-center text-xs ${
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-slate-900/90 border border-slate-800 rounded-xl pl-10 pr-3.5 py-3 text-white text-xs placeholder:text-slate-500 outline-none focus:border-gold focus:ring-1 focus:ring-gold transition-all"
                {...register('identifier', { required: `${loginMethod === 'email' ? 'Email' : 'Phone number'} is required` })}
              />
            </div>
          </div>
          {errors.identifier && (
            <p className="text-[11px] text-rose-400 mt-1">{errors.identifier.message}</p>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-900/90 border border-slate-800 rounded-xl pl-10 pr-10 py-3 text-white text-xs placeholder:text-slate-500 outline-none focus:border-gold focus:ring-1 focus:ring-gold transition-all"
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="group w-full py-3.5 rounded-xl font-bold text-black bg-gold hover:bg-amber-400 cursor-pointer shadow-md text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-colors mt-6"
        >
          {isLoading ? (
        ...
        <div className="pt-2 mt-8">
          <p className="text-[10px] text-center text-slate-500 uppercase tracking-wider mb-3 font-bold">Or Sign In With</p>
          <div className="grid grid-cols-2 gap-2.5">
            <button
              type="button"
              onClick={() => addToast({ type: 'info', title: 'Google Sign In', message: 'Google authentication demo.' })}
              className="py-2.5 px-3 rounded-xl bg-slate-900 border border-slate-800 text-slate-300 hover:text-white hover:border-slate-700 transition-all flex items-center justify-center gap-2 text-xs font-medium cursor-pointer shadow-sm"
            >
        ...
        <div className="pt-6 mt-8 border-t border-white/5 space-y-4">
          ...
          <div className="flex items-center justify-center gap-3 text-[11px] text-slate-500 font-medium pt-2 flex-wrap">
            <span className="flex items-center gap-1">
              <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" /> SHA-256 Audited
            </span>
            <span>•</span>
